...
while True:
    try:
        ...
        if result.multi_hand_landmarks:
            ...
            # )
            # scroll_up_active = thumb_dir == -1 and non_thumb_fingers_folded and (not gesture_features["pinch_index_thumb"])
            ...
            if fist_active:
                ...
                # if scroll_active and (frame_time - last_scroll_time) >= SCROLL_INTERVAL:
                #     if scroll_up_active:
                #         pyautogui.scroll(SCROLL_STEP)
                #         set_state("scroll_up", "Thumbs-up detected - scrolling up")
                #     else:
                #         pyautogui.scroll(-SCROLL_STEP)
                #         set_state("scroll_down", "Thumbs-down detected - scrolling down")
                #     last_scroll_time = frame_time
                ...
                # elif scroll_active:
                #     # Keep cursor steady while thumb scrolling is active.
                #     prev_px, prev_py = avg_x, avg_y
                #     prev_hand_x, prev_hand_y = avg_x, avg_y
                #     residual_move_x_px, residual_move_y_px = 0.0, 0.0
                else:
